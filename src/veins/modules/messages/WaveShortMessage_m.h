 ...
 *     double Data = "";
 * 
 * 	// A vehicle sends 2 message types and a RSU sends 1 message type
 * 	// So these message shold be identified at the receiver. for that the "Psid" is needed.
 * 
 * 	// in Vehicle,
 * 	// REQUEST = 1 -> is to request a lock on all 4 lanes that needed to be locked  if the vehicle is PASSING
 * 	// RELEASE = 2 -> is to release the locks that are requested.
 * 
 * 	// in RSU,
 * 	// PERMIT  = 3 -> is a broadcast message that says all the vehicles in the "plt" to move on.
 * 
 * 	// Each message type has it's own data.
 * 	// We've decided to add all data to wsmData. It is defined above.
 * 
 * 	// REQUEST(i, lid_of_i);
 * 	//		i = Vehicle ID
 * 	//		lid_of_i = Lane no of vehicle i
 * 
 * 	// RELEASE (i, lid_of_i);
 * 
 * 	// PERMIT(plt)
 * 	//		plt = list of vehicle that can pass (basically, 3 vehicles at a time)
 * }
 * </pre>
 */
